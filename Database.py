...
def checkdatabase():
    ...
    sql = 'CREATE TABLE IF NOT EXISTS all_data(idx INTEGER NOT NULL, title TEXT, age_min INTEGER, age_max INTEGER, area_code INTEGER, area_list TEXT, work TEXT, type TEXT, area TEXT, host TEXT, href TEXT, PRIMARY KEY("idx" AUTOINCREMENT))'
...
def InsertData(youth, alldata, index_num):
    ...
    '''
    cursor.execute(
        "INSERT INTO all_data(title, age_min, age_max, area_code, area_list, work, type, area, host, href) SELECT * FROM youth LEFT OUTER JOIN all_data ON youth.title = all_data.title WHERE all_data.title IS NULL;"
    )
    cursor.commit()
...
checkdatabase()
#cursor.execute("INSERT INTO all_data(title, age_min, age_max, area_code, area_list, work, type, area, host, href) SELECT title, age_min, age_max, area_code, area_list, work, type, area, host, href FROM youth")
UpdateAlldata()
conn.commit()
