...
def youth():
    # 크롬 드라이버 저장된 위치
    cromedriver_path = 'C:\Users\user-pc\SSF\chromedriver.exe'
    ...
    for page in range(1, page_num + 1):
        ...
        for index in range(1, len(num) + 1):
            ...
            DF_youth.loc[len(DF_youth.index)] = {'title': title, 'href': href, 'age_min': age_min, 'age_max': age_max,
                                                 'area': area, 'work': work, 'type': type_str, 'host': host}

            URL = 'https://www.youthcenter.go.kr/youngPlcyUnif/youngPlcyUnifList.do?pageIndex=' + str(
                page) + '&pageUnit=60'
            driver.get(url=URL)
            ...
            if find_index >= 0:
                ...
                for overlap_index in range(len(overlap_check.index)):
                    if host_check[index] == overlap_check['overlap'][overlap_index]:
                        if area_text[find_index - 1:find_index + len(host_check[index])].find(
                                overlap_check['notOverlap'][overlap_index]) >= 0:
                            is_overlap = 1
    ...
    final_df = DF_youth[
        ['title', 'age_min', 'age_max', 'area_code', 'area_list', 'work', 'type', 'area', 'host', 'href']]
